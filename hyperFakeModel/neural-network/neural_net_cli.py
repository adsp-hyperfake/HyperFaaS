...
def _read_and_validate_hyperparams(file_path):
    ...
    assert isinstance(data["hidden_dims"], list), "hidden_dims should be a list"
    assert isinstance(data["dropouts"], list), "dropouts should be a list"
    assert len(data["hidden_dims"]) == len(data["dropouts"]), (
