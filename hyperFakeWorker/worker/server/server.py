...
def serve():
    ...
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=multiprocessing.cpu_count()*2))
    ...
    try:
        while True:
            time.sleep(6)
            logger.info(f"Active Functions: {controller_servicer._fn_mngr.num_active_functions}")
